...
void *TrainModelThread(void *id) {
  ...
  long long *c_dropout = (long long *) calloc(c_length, sizeof(long long));
...
#ifdef DROPOUT
      dropoutLeft = 1;
      for (++i; i < cur_ins->c_num; ++i) {
        NRAND
        if (next_random % DROPOUTRATIO >= dropout) { //notdropout
          dropoutLeft += 1;
          l1 = cur_ins->cList[i] * c_length;
          for (j = 0; j < c_length; ++j) if (c[l1+j] > c_error[j]) {
            c_error[j] = c[l1 + j];
            c_dropout[j] = i;
          }
        }
...
#ifdef DROPOUT
      for (a = 0; a < c_length; ++a) if (c_dropout[a] >= 0) {
        l1 = cur_ins->cList[c_dropout[i]] * c_length;
        c[l1 + i] += GCLIP(c_error[i]);
      }  
#else
  ...
  if (0 != ignore_none) {
    ...
    for (i = 0; i < val_ins_num; ++i){
      ...
      if (cur_ins->supList[0].label == NONE_idx)
        continue;
      l1 = c_length * cur_ins->cList[0];
      for (j = 0; j < c_length; ++j)
        cs[j] = c[l1 + j];
      for (a = 1; a < cur_ins->c_num; ++a) {
        l1 = c_length * cur_ins->cList[a];
        for (j = 0; j < c_length; ++j) if (c[l1 + j] > cs[j])
          cs[j] = c[l1 + j];
      }
