...
char save_representation[MAX_STRING];
...
void SaveRepresentation() {
  FILE *fo = fopen(save_representation, "w");
  fprintf(fo, "%lld %lld %lld %lld\n", l_length, c_size, l_size, d_size);
  long long a, b, j, l1, l2;
  real g;
  for (a = 0; a < c_size; ++a){
    l1 = a * c_length;
    for (b = 0; b < l_length; ++b){
      g = 0;
      l2 = b * c_length;
      for (j = 0; j < c_length; ++j) g += c[l1 + j] * o[l2 + j];
#ifdef ACTIVE
      if (g < -MAX_EXP) g = -1;
      else if (g > MAX_EXP) g = 1;
      else g = tanhTable[(int)((g + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
#else
      g = g;
#endif
      fprintf(fo, "%f ", g);
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-output", argc, argv)) > 0) strcpy(save_representation, argv[i + 1]);
  ...
  EvaluateModel();
  if (debug_mode > 1) printf("Save Representation \n ");
  SaveRepresentation();
