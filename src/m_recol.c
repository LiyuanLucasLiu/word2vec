...
long long  *cCount;
int binary = 1, debug_mode = 2, reSample = 20, min_count = 5, num_threads = 1, min_reduce = 1, infer_together = 0, special_none = 0, no_lb = 1, no_db = 1, ignore_none = 0, error_log = 0;
...
void InitNet() {
  ...
  for (b = 0; b < l_size; ++b) for (a = 0; a < l_length; ++a)
    l[b * l_length + a] = (rand() / (real)RAND_MAX - 0.5) / l_length;
...
void TrainModel() {
  ...
  tot_c_count = 0;
  memset(cCount, 0, c_size);
  printf("shuffling and building sub-sampling table\n");
  if (negative > 0) {
    for (a = 0; a < ins_num; ++a) {
      //shuffle
      // NRAND
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./mrecol -train /shared/data/ll2/CoType/data/intermediate/KBP/train.data -output /shared/data/ll2/CoType/data/intermediate/KBP/class.model -threads 16 -binary 0 -NONE_idx 6 -cleng 30 -lleng 50 -lambda1 3 -resample 40 -ignore_none 1 -error_log 1 2> log.txt\n\n");//-none_idx 5 
    return 0;
  }
  output_file[0] = 0;
  // save_vocab_file[0] = 0;
  ...
  if ((i = ArgPos((char *)"-error_log", argc, argv)) > 0) error_log = atof(argv[i + 1]);
